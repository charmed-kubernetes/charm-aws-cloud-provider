...
async def test_build_and_deploy(ops_test):
    charm = next(Path(".").glob("aws-cloud-provider*.charm"), None)
    if not charm:
    ...
    await ops_test.model.block_until(
        lambda: "aws-cloud-provider" in ops_test.model.applications, timeout=60
    )
...
async def test_provider_ids(kubernetes):
    async for node in kubernetes.list(Node):
        assert node.spec.providerID.startswith("aws://")
